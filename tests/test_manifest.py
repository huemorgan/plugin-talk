...
PKG = Path(__file__).parent.parent / "plugin_talk"
...
def test_settings_tab_declared():
    ...
    assert len(tabs) == 1
    assert tabs[0].id == "talk"
    assert tabs[0].iframe_src == "/api/p/plugin-talk/ui/settings/"


def test_widget_files_shipped():
    """The widget's static files exist inside the package (survive packaging)."""
    widget_dir = PKG / "ui" / "widgets" / "talk"
